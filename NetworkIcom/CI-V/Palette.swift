...
func setColors(palette: [[UInt8]]) -> [PixelColor] {
    ...
    for n in 0..<8 {
        for i in 0..<32 {
            let r = Double(palette[n][0]) + (Double(i) * (Double(palette[n+1][0]) - Double(palette[n][0])) / 32)
            let g = Double(palette[n][1]) + (Double(i) * (Double(palette[n+1][1]) - Double(palette[n][1])) / 32)
            let b = Double(palette[n][2]) + (Double(i) * (Double(palette[n+1][2]) - Double(palette[n][2])) / 32)
            result[i + 32 * n].r = UInt8(r)
            result[i + 32 * n].g = UInt8(g)
            result[i + 32 * n].b = UInt8(b)
        }
    }
    return result
}
